...
def sample(opt):
    ...
    images = torch.cat((x, y, y_DM)).cpu()
    images = apply_colormap(images, vmin=0, vmax=1)
    seg = torch.cat((y_task, y_DM_task)).cpu()
    seg = combine_purple_green(seg[:,1:2], seg[:,0:1])
    grid = make_grid(torch.cat((images, seg)), nrow=n_samples)
    ...
    save_image(grid, f"{save_dir}/samples.png")
    ...
    for i in range(n_samples):
        ...
        save_tensor_as_tif(y_DM[i], f"{tifs_dir}/valid_{indices[i]}_DM.tif")
